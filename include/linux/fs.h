 ...
 * of the 'struct inode'
 * 保持大部分为只读且经常访问（尤其是*开头的RCU路径查找和'stat'数据）字段“结构信息节点”的*
 */
struct inode {
	...
	struct file_lock_context *i_flctx;				/* i_flock; 这个为啥名字不一样?   文件锁链表 */
	...
	void *i_private; /* fs or device private pointer 用户私有数据*/
} __randomize_layout;
...
#define SB_KERNMOUNT (1 << 22) /* this is a kern_mount call 这是一个 kern_mount 调用*/
...
struct super_block {
	...
	*/
	...
	/**
	 * 文件系统类型(也就是当前这个文件系统属于哪个类型？ext2还是fat32)，
...
#define FS_REQUIRES_DEV 1			//这种类型的文件系统必须位于物理磁盘设备上.
...
#define FS_RENAME_DOES_D_MOVE                                                  \
	...
	*/
	...
	/**
	 * 指向文件系统类型链表中,下一个元素的指针.
	*/
	struct file_system_type *next;

	/*
	*	表示给定类型的已安装文件系统所对应的超级块链表的头
	*/
